...
## [2026-09-18] 任务: 双TMC5160 速度极限/边界/加速度全链路验证 — /cl end 验收通过
- **模式**: /cl run（全自动闭环；多阶段：速度二分→边界收敛→加速度上推→生产固化）
- **现象**: 24V+传送带，U2=弱电机。vel模式RTT enc冻结判据有效，二分收敛U2∈(1462,1481]rpm/U1∈(1500,1572]rpm。端序坑：用户帧BE读错导致-939M/367s假失步，实为PCAN transient（stop/rel-ALL/单电机复测全通）。ENC_TOLERANCE 256→512解决G6 1442rpm巡航跟随滞后263µsteps误报。失步=永久XACTUAL偏移，每次失速须24V断电重建。ST-Link失联（OpenOCD "open failed"），全程J-Link烧录。
- **尝试**:
  - 速度二分：vel模式+RTT enc冻结→1350双过→1425双过→1462双过→1481U2丢U1过，收敛到U2∈(1462,1481]/U1∈(1500,1572]rpm
  - 端序定位：PCAN用户帧BE读错（-939M），修正后LE帧与固件一致；"丢步"实为PCAN transient（同窗stop/rel-ALL复测全通）
  - ENC_TOLERANCE：G6 1442rpm巡航enc lag=263µsteps>256阈值误报失步，升到512后复测干净；刻意失速验证1572rpm仍正确检测
  - Group6定值：1442rpm=1471×0.98（U2弱电机边界内），用户10+次复测无异常
  - 加速度上推：40000全组零丢步×6组×4腿=24腿全干净（含G6@30000/@40000各4腿）；G5/G6行程时间严格单调递减验证AMAX生效
  - 固化：G1~G5=28000（×0.7）,G6=39200（×0.98）,DMAX=AMAX对称
- **最终方案**: VMAX冻结（G1:5000/G2:20000/G3:50000/G4:100000/G5:572303/G6:1375452），AMAX=DMAX（G1~G5:28000/G6:39200），ENC_TOLERANCE=512，Group6生产禁用param=6+值守+用后查失步位
- **验证结果**: ✅ 二分边界收敛（双电机失步确认）；✅ 加速度终验12/12腿bit0=1无失步；✅ G5行程1.28→1.22s/G6 1.27→1.18s；format 0FAIL/build 0E/0W/FLASH 44636B
- **经验引用**: PCAN transient≠固件bug（工具坑→run-fault.md）；ENC_TOLERANCE必须覆盖最大巡航滞后（263→512µsteps）；开环失步=永久偏移须断电重建（非stop可恢复）；AMAX 40000全组干净但真临界未知（收益递减边界G1仅0.03%加速占比）
- **提示词压缩**: `[STM32H750/TMC5160 24V传送带] [vel二分+RTT enc冻结判据] [U2∈(1462,1481] U1∈(1500,1572] 600rpm裕量2.4×] [AMAX×0.7=28000/G6×0.98=39200 终验12/12] [J-Link烧录+PCAN transient≠固件]`
- **违规自检**: ✅已读 run+codegen+watcher+preflight+probe+tim+build+flash+format-gate+risk+run-fault+end；✅全自动闭环；✅澄清+用户裁决后才生成；✅工具问题按run-fault处理；违规记录：无
