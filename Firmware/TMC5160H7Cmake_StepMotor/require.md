...
烧录工具: ST-Link + OpenOCD (首选) / J-Link (备选)
烧录指令: python tools/openocd_flash.py --flash <工程根>\build_cl\TMC5160H7_StepMotor.hex --target stm32h7x
J-Link 烧录: python tools/jlink_flash.py --flash <工程根>\build_cl\TMC5160H7_StepMotor.hex --target stm32h750vb
...
| 2026-09-18 | 组5超高速 | 硬件时序/电气 | module/drv/tmc5160.c:94 | 组5 VMAX=2863311(50rps)在24V+传送带下必失速丢步（实测边界1200<stall≤1500rpm，U2先丢），丢步后开环位置丢失、无限位则无法0x09恢复，只能断电重建零点 | 实测 | [x] 已解决（组5已降600rpm生产档并验证PASS） |
| 2026-09-18 | 组6极限运行 | 失败路径/恢复 | module/drv/tmc5160.c:95 | 组6=1442rpm逼近U2失速边界(1471,1481]，属故意高风险运行：失速→位置丢失→断电重建；仅有人值守测试，生产帧禁用param=6，每次用后查失步位 | 推演链 | [✓] 人工确认（用户10+次复测干净，生产禁用+值守+用后查失步位三条防线已固化） |
...
| 主循环单圈 | main while(1) tag2全事务/tag5心跳块 | 空载循环体约1us；1Hz心跳块含12×SPI读约625us | —(无周期条目，不设预算) | 软件(DWT 480MHz+RTT Ch0)·抽稀500:1 | 典型1us(p50)/max2us(1105cyc,2928样)；心跳圈625us/300367cyc(1Hz,3样)(2026-09-18) | — | [x] 串口不可用改RTT回传；探针常驻生产版空宏(44600B一致) |
...
> 状态标记: [ ] 未开始 / [x] AI 完成(待验收) / [✓] 人工验收通过 / [!] 阻塞 / [c] Code Only。**滚动窗口**：仅保留 未完成 + 最近 5 条已完成（更早删除；长链在 `.cl/experience/log.md`，负结论在 `memory`「定论」段，全文 git 兜底）。

## 2026-09-18
- [c] RTT 调试通道切换（UART→RTT）：cl_config.h UART_DBG→0/RTT_DBG→1 + main.c 启用 RTT_DBG_Init + rtt_dbg.c 追加 printf retarget 到 RTT Channel 0（_write 覆盖 syscalls.c weak）（起 2026-09-18 | 止 2026-09-18 | 验收 build 0E/0W FLASH 44600B）风险:无
- [✓] 丢步位置恢复（断电重建零位）：24V重上电后U2即干净，U1失步锁存又经3轮stop读后自清；单电机rel±验证双电机终态bit0=1无失步；rel-U1两次丢帧经对照实为PCAN transient（同窗stop/rel-ALL/单电机复测全通），非固件通路（起 2026-09-18 | 止 2026-09-18 | 验收 双电机终态干净+单步 motion OK）风险:无
- [✓] 6组加减速优化（VMAX冻结；G1~G5取40000试探干净×0.7=28000，G6取40000×0.98=39200；DMAX对称跟随；正反各2趟+终验12腿全bit0=1无失步；G5行程1.28→1.22s/G6 1.27→1.18s符合加速理论；40000以上未探留容缺）（起 2026-09-18 | 止 2026-09-18 | 验收 终验12/12 PASS）风险:无
- [✓] 精测边界+Group6固化（U2正反转边界收敛(1462,1481]，U1(1500,1572]；Group6=1442rpm=1471×0.98；U1精确边界留容缺）（起 2026-09-18 | 止 2026-09-18 | 验收 边界收敛+G6 PASS）风险:见风险日志"组6极限运行"行
- [✓] 最高不卡死速度二分（vel+RTT enc冻结判据）：U2∈(1462,1481]rpm、U1∈(1500,1572]rpm，U2为弱电机；生产600rpm裕量2.4倍以上（起 2026-09-18 | 止 2026-09-18 | 验收 边界收敛+双电机失步确认）风险:无
- [✓] 组5处置固化（s_profiles[4]→600rpm VMAX=572303/AMAX=28000+推导注释）（起 2026-09-18 | 止 2026-09-18 | 验收 PASS）风险:无
...
| 6 | param | 运动参数组 ID 1~6（cmd 0x06 时为 PID 参数类型；0x09 时预留忽略；组6=1442rpm 极限运行档，生产禁用，见组6注记） |
| 7 | checksum | 见上 |
...
| 0x04 | 速度模式 | 目标速度=斜坡寄存器原值（符号=方向，+正/−反；物理µsteps/s=值×fCLK/2^24，fCLK=15MHz时物理=值/1.1178；如600rpm=512000µsteps/s对应值572303，实测闭合2026-09-18） | 持续旋转 | 收到即回一帧（无目标，无终态） |
